...
**another thing:** The score in BedGraph for peak calling is signal in each 100-bp bin, so the local lambda should be the average signal on 100-bp level. Then we can test signal at each 100-bp bin against the local lambda with Poisson distribution. Although the default behavior in MACS2 is testing at each basepair, our each position in a 100-bp bin will generate the same p-value and call be merged again. Therefore, this process still can be throught as test at each 100-bp bin and reported
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEyMTI2NDE3ODIsMjg4NjEyNDY3LDIwOD
